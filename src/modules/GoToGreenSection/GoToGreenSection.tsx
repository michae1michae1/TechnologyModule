...
import { ChevronUp, ChevronDown, CheckCircle2, AlertTriangle, Zap } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        <div className="col-span-2">
          <div className="bg-slate-800 p-4 rounded-lg h-full">
            <h3 className="text-lg font-medium mb-3">Installation Status</h3>
            
            <div className="overflow-y-auto max-h-[300px] pr-1 rounded-md">
              <table className="w-full">
                <thead className="sticky top-0 bg-slate-800 z-10">
                  <tr className="border-b border-slate-700">
                    <th className="text-left py-2 px-2 text-xs uppercase tracking-wider text-slate-400">Installation</th>
                    <th className="text-center py-2 px-2 text-xs uppercase tracking-wider text-slate-400">To Green</th>
                    <th className="text-center py-2 px-2 text-xs uppercase tracking-wider text-slate-400">Gap Level</th>
                    <th className="text-center py-2 px-2 text-xs uppercase tracking-wider text-slate-400">Technologies</th>
                  </tr>
                </thead>
                <tbody>
          ...
          {!selectedInstallation ? (
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="bg-slate-800 p-4 rounded-lg">
                  ...
                  <div className="flex items-center">
                    <div className="w-full bg-slate-700 rounded-full h-4 mr-2">
                      <div 
                        className={`h-4 rounded-full ${installationScore >= 80 ? 'bg-green-500' : installationScore >= 50 ? 'bg-yellow-500' : 'bg-red-500'}`}
                ...
                <div className="bg-slate-800 p-4 rounded-lg">
                  <h3 className="text-lg font-medium mb-2">Gap to Green</h3>
                  <div className="flex items-center">
                    ...
                    {!canReachGreen && (
                      <div title="Not enough technology impacts to reach green status">
                        <AlertTriangle size={24} className="text-yellow-500 ml-2" />
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div className="bg-slate-800 p-4 rounded-lg">
                ...
                <div className="overflow-y-auto max-h-[250px] pr-1 rounded-md">
                  <table className="w-full">
                    <thead className="sticky top-0 bg-slate-800 z-10">
                      <tr className="border-b border-slate-700">
                        <th className="text-left py-2 px-3 text-xs uppercase tracking-wider text-slate-400">Technology</th>
                        <th className="text-center py-2 px-3 text-xs uppercase tracking-wider text-slate-400">Vendor</th>
                        <th className="text-center py-2 px-3 text-xs uppercase tracking-wider text-slate-400">Status</th>
                        <th className="text-center py-2 px-3 text-xs uppercase tracking-wider text-slate-400">Impact</th>
                        <th className="text-right py-2 px-3 text-xs uppercase tracking-wider text-slate-400">Cost</th>
                      </tr>
                    </thead>
                    <tbody>
                      {technologiesNeededForGreen.map(tech => (
